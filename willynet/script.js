...
// Easter Egg - Watch Dogs inspired keyboard combo (↑↓↑↓CTOS)
let easterEggKeys = [];
const easterEggSequence = ["ArrowUp", "ArrowDown", "ArrowUp", "ArrowDown", "KeyC", "KeyT", "KeyO", "KeyS"];
...
document.addEventListener("keydown", (e) => {
    ...
    // Add key to sequence
    easterEggKeys.push(e.code);
    
    // Keep only the last 8 keys (length of sequence)
    if (easterEggKeys.length > 8) {
        easterEggKeys.shift();
    }
    ...
    if (easterEggKeys.length === 8) {
        let matches = true;
        for (let i = 0; i < 8; i++) {
            if (easterEggKeys[i] !== easterEggSequence[i]) {
                matches = false;
                break;
            }
        }
...
function showEasterEgg() {
    const modal = document.getElementById("watchDogsEasterEgg");
    if (modal) {
        modal.style.display = "block";
        
        // ctOS access sequence
        console.log("%c[ctOS 2.0] Unauthorized access attempt detected...", "color: #00d4ff; font-weight: bold; font-size: 14px;");
        setTimeout(() => {
            console.log("%c[ctOS 2.0] Running security bypass protocols...", "color: #00ff88; font-weight: bold;");
        }, 1000);
        setTimeout(() => {
            console.log("%c[ctOS 2.0] Firewall breach detected - escalating...", "color: #ff6b47; font-weight: bold;");
        }, 2000);
        setTimeout(() => {
            console.log("%c[ctOS 2.0] Access granted - Welcome to WillyNet surveillance network", "color: #47ffff; font-weight: bold;");
            console.log("%c[ctOS 2.0] All network nodes under surveillance", "color: #00ff88;");
        }, 3000);
...
function initCtOSTabs() {
    const menuItems = document.querySelectorAll('.ctos-menu-item');
    const tabs = document.querySelectorAll('.ctos-tab');
